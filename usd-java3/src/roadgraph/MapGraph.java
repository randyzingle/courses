...
public class MapGraph {
	...
	public void addEdge(GeographicPoint from, GeographicPoint to, String roadName, String roadType, double length)
		...
		// input data looks fine, let's make the Edge and add it to the
		// adjacency list
		nedge++;
		EdgeNode edge = new EdgeNode(to, roadName, roadType, length);
	...
	public List<GeographicPoint> bfs(GeographicPoint start, GeographicPoint goal,
		...
		// If the starting point is the ending point return a list with the
		// start point
		if (start == goal) {
			path.add(start);
			return path;
		...
		while (!candidate.isEmpty()) {
			...
			ArrayList<EdgeNode> edges = adjList.get(curr);
			for (EdgeNode edgePoint : edges) {
				GeographicPoint pt = edgePoint.getLocation();
	...
	private List<GeographicPoint> getPath(HashMap<GeographicPoint, GeographicPoint> path, GeographicPoint goal,
	...
	public List<GeographicPoint> dijkstra(GeographicPoint start, GeographicPoint goal,
		...
		EdgeNode startNode = new EdgeNode(start, null, null, 0);
		startNode.setDistanceFromStart(0);
		EdgeNode goalNode = new EdgeNode(goal, null, null, 0);
		EdgeNode curr = null;
		...
		while (!candidates.isEmpty()) {
			...
			ArrayList<EdgeNode> nodeList = adjList.get(curr.getLocation());
	...
	public String toString() {
		...
		for (GeographicPoint key : adjList.keySet()) {
			...
			ArrayList<EdgeNode> eList = adjList.get(key);
			for (EdgeNode edge : eList) {
				buf.append("(" + edge.getLocation() + ") ");
			}
			buf.append("\n");
		}
		return buf.toString();
